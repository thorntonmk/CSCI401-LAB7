...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
   bool isFree = list_is_in_by_size(freelist, blocksize);
    if(isFree) {
        int idx = list_get_index_of_by_Size(freelist, blocksize);
        block_t *current_block = list_remove_at_index(freelist, idx);
        int block_end = current_block->end;
        current_block->pid = pid;
        current_block->end = current_block->start + blocksize - 1;
        list_add_ascending_by_address(alloclist, current_block);
        //check if there's any fragments left and add to free
        if(block_end - current_block->end + 1 > 0) {
            block_t *blk = malloc(sizeof(block_t));
            blk->pid = 0;
            blk->start = current_block->end + 1;
            blk->end = block_end;
            if(policy == 1) {
                ...
                printf("Invalid policy %d passed.\n", policy);
            }
        }
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
   bool isAlloc = list_is_in_by_pid(alloclist, pid);
   if(!isAlloc) {
    printf("Error: Can't locate Memory Used by PID: %d\n", pid);
   } else {
    bool idx = list_get_index_of_by_Pid(alloclist, pid);
    block_t *removed = list_remove_at_index(alloclist, idx);
    removed->pid = 0;
    if(policy == 1) {
        list_add_to_back(freelist, removed);
    } else if(policy == 2) {
        list_add_ascending_by_blocksize(freelist, removed);
    } else if(policy == 3) {
        list_add_descending_by_blocksize(freelist, removed);
    } else {
        printf("Invalid policy %d passed.\n", policy);
    }
   }
